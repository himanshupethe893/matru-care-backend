...
// REVISED: Route for a doctor to get their assigned patients with last message and unread count
app.get('/api/doctor/patients', auth, async (req, res) => {
    try {
        const doctorUserId = req.user.id;

        const doctor = await DoctorDetails.findOne({ userId: doctorUserId });
        if (!doctor) {
            return res.status(404).json({ msg: 'Doctor details not found for this user.' });
        }
        ...
        const patientsWithChatInfo = await Promise.all(patients.map(async (patient) => {
            const conversation = await ChatConversation.findOne({
        ...
        // Sort patients to show those with unread messages first, then by last message time
        patientsWithChatInfo.sort((a, b) => {
...
app.post('/api/chat/send', auth, async (req, res) => {
    ...
    try {
        ...
        const chatMessage = new ChatMessage({
            ...
            receiver: receiverId,
            message: encryptedMessage,
            // 'read' will default to false, which is what we want.
        });
        ...
        const populatedMessage = await chatMessage.populate('sender', 'name');
        ...
        io.to(receiverId).emit('receive-chat-message', {
            ...
            message, // Send decrypted message over socket
